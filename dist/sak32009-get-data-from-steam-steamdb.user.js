...
(function(CryptoJS2, $2, bootstrap$1, lodash, sprintfJs) {
  "use strict";
...
`;
  ...
  const steamtools = `INSTALL/UNLOCK APP:
[data]appId[/data] = [data]name[/data]
  ...
  class GetDataFromSteamSteamDB {
    ...
    getDataFromSteamDBApp() {
      ...
      $2("#dlc.tab-pane tr.app[data-appid]").each((_index, element) => {
        const dom = $2(element);
        const appId2 = dom.attr("data-appid");
        const $appName = dom.find("td:nth-of-type(2)");
        const appName = lodash.escape(lodash.unescape($appName.text().trim()));
        if (element.hasAttribute("hidden")) {
          this.extractedData.dlcsHiddens[appId2] = appName;
          this.extractedData.countDlcsHiddens += 1;
        } else if ($appName.hasClass("muted")) {
          this.extractedData.dlcsUnknowns[appId2] = appName;
          this.extractedData.countDlcsUnknowns += 1;
        } else {
          this.extractedData.dlcs[appId2] = appName;
    ...
    getDataFromSteamPowered() {
      ...
      $2("a.game_area_dlc_row").each((_index, element) => {
        const dom = $2(element);
        const appId2 = dom.attr("data-ds-appid");
...
`;
    ...
    setModalContainer() {
      ...
      const extended = sprintfJs.sprintf(skExtendedHtml, {
        ...
        condDlcsHiddens: isSteamDBApp && this.extractedData.countDlcsHiddens > 0 ? "" : "hidden",
        listDlcsHiddens: Object.keys(this.extractedData.dlcsHiddens).join(", ")
      });
    ...
    showOutputOnTextarea(fileName, out) {
      ...
      $2(".sak32009 textarea#sake_output").val(out).scrollTop(0);
    }
    setEvents() {
          ...
          if (selectedOption === "greenLuma2020ManagerBlueAmulet" || selectedOption === "steamLauncherMiniOnlyDlcsList") {
            ...
            if (selectedOption === "greenLuma2020ManagerBlueAmulet") {
              outParse.forEach((values, key) => {
                ...
                outParse[key].name = lodash.unescape(appName);
              });
            } else {
              Object.keys(outParse).forEach((key) => {
                const appName = outParse[key];
                outParse[key] = lodash.unescape(appName);
              });
            }
            out = JSON.stringify(outParse, void 0, 4);
          } else if (selectedOption === "appIdAppIdName2") {
            out = out.replaceAll("&quot;", '\\"');
            out = lodash.unescape(out);
          } else {
            out = lodash.unescape(out);
          }
